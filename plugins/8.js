//كود لعبة بومب مقدم من قناه كيلوا ملف الاجابه
...
export async function before(m) {
    try {
        ...
        if (isSurrender) {
            await this.reply(m.chat, ` استسلم :(`, m);
        ...
        if (this.bomb[id] && m.quoted && m.quoted.id == this.bomb[id][3].id && !isNaN(body)) {
            ...
            if (json.emot == '💥') {
                ...
                teks += `*انتهت اللعبة!*، يُفتح الصندوق الذي يحتوي على القنبلة : (- *${formatNumber(reward)}*)`;
                ...
                if (open >= 8) {
                    let teks = `乂  *B O M B*\n\n`;
                    teks += `Kirim angka *1* - *9* لفتح *9* مربع الأرقام أدناه :\n\n`;
