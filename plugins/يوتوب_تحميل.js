...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  if (!args[0]) throw '[❗شرح❗] هذا الامر خاص بي تحميل المقاطع من منصة اليوتوب';
  let enviando;
  if (enviando) return  
      enviando = true    
  let youtubeLink = '';
  ...
  const { key } = await m.reply(`*⌛ جاري التحميل ⏳*`);
  try {
   ...
   if (fileSizeInMB > 100) {
    await conn.sendMessage(m.chat, {document: buff, caption: `*▢ التاريخ:* ${ttl_1}\n*▢ حجم المقطع:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `*[ ✅ ] تم تحميل الفيديو بي نجاح.*\n\n*—◉ Se envío en formato de docuemnto debido a que el video pesa ${roundedFileSizeInMB} MB y supera el limite establecido por WhatsApp.*\n*◉ Titulo:* ${ttl_1}`, edit: key}, {quoted: m});
    enviando = false
   } else {
    await conn.sendMessage(m.chat, {video: buff, caption: `*▢ التاريخ:* ${ttl_1}\n*▢ حجم المقطع:* ${roundedFileSizeInMB} MB`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
    await conn.sendMessage(m.chat, {text: `*[ ✅ ] تم تحميل الفيديو بي نجاح .*`, edit: key}, {quoted: m});
    enviando = false   
   }
 } catch (ee) {
    console.log(ee)
  try {
    ...
    try {
      ...
      await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `『𝑅𝐼𝑇𝐴🌸𝐵𝛩𝑇』`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
      await conn.sendMessage(m.chat, {text: '*[ ✅ ] تم تحميل الفيديو بي نجاح.*', edit: key}, {quoted: m});
      enviando = false
    } catch {
      try {
        ...
        await conn.sendMessage(m.chat, {text: `*[ ❌ ] الفيديو لم يتم تحميله بي نجاح.*`, edit: key}, {quoted: m});
        throw '*حدت خطأ اين رابط المقطع .*';
      }
    }
  }
}};
handler.command = /^(يوتوب|يوتيوب|dlmp4|getvid|yt(v|mp4)?)$/i;
