...
const handler = async (m, { command, usedPrefix, conn, text }) => {
  if (!text) throw `*هذا الامر خاص بي تحميل مقاطع يوتوب بي الصوت او الصوت والصورة*\n\n*🌟متال* _${usedPrefix + command} *استخدم هذا الامر لمشاهدة المقطع بي الصوت والصورة*\n\n*🌟 متال:* _${usedPrefix + command} *رابط المقطع*`;
if (enviando) return;
    enviando = true
  try {
    const apiUrls = [
    ...
    if (!data.resultado || !data.resultado.url) {
      ...
      try {      
        if (command === 'يوتوب_صوت') { // play.1 con CFROS API v1 ytmp3
              apiUrl = `https://api.cafirexos.com/api/v1/ytmp3?url=${data.resultado.url}`;
              mimeType = 'audio/mpeg';
              fileName = 'error.mp3';
              buff = await conn.getFile(apiUrl);          
            } else if (command === 'يوتوب_صوت2') { // play.2 con CFROS API v1 ytmp4
          ...
          try {
            if (command === 'يوتوب_صوت') { // play.1 con CFROS API v2 ytmp3
              apiUrl = `https://api.cafirexos.com/api/v2/ytmp3?url=${data.resultado.url}`;
              mimeType = 'audio/mpeg';
              fileName = 'error.mp3';
              buff = await conn.getFile(apiUrl);              
            } else if (command === 'يوتوب_صوت2') { // play.2 con CFROS API v2 ytmp4
    ...
    const dataMessage = `_*『𝑅𝐼𝑇𝐴🌸𝐵𝛩𝑇』*_\n\n▢ *العنوان:* ${data.resultado.title}\n\n▢ *التاريخ:* ${data.resultado.publicDate}\n\n▢ *المغني:* ${data.resultado.channel}\n\n▢ *رابط المقطع:* ${data.resultado.url}`;
...
handler.command = ['يوتوب_صوت', 'يوتوب_صوت2'];
